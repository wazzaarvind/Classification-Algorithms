...
def numericalData(allRecords, ignoreIndex, totalEntropy, runIndex):
	#
	### For NUMERICAL DATA ###
	#
	...
	listObjects = list(set(runIndex) - set(ignoreIndex))
...
def findBestGain(recordsToRun, listCatIndex, attributeChoice): # Function accepts a numpy.ndarray() and a list with "object" attributes index
	...
	# Numerical Portion
	#numtime = time.time()
	funcMaxGain, funcChosenMidValue, funcChosenAttributeIndex = numericalData(recordsToRun, listCatIndex, totalEntropy, attributeChoice)
	#print("Numerical Data: ",time.time() - numtime)
...
def runTreeBuild(treeDepth, parentNode, listOfCats, allRecords): # allRecords is of type numpy.ndarray()
	...
	# Continue execution of the node is IMPURE
		...
		noOfATT = int(0.2 * allRecords.shape[1])
		
		attributeChoice = np.random.choice(range(0,allRecords.shape[1]-1),noOfATT,replace=False)
		attributeChoice = list(attributeChoice)
		#print(type(attributeChoice))
		attributeChoice.sort()
		#print(attributeChoice)

		listCatIndex = list(set(attributeChoice).intersection(listOfCats))

		#print("Tree: listCatIndex: ",listCatIndex)
		#print("Tree: attributeChoice: ",attributeChoice)

		#tempAllRecords = allRecords[:,attributeChoice]
		#attributeChoice = [range(0,len(attributeChoice))]

		midMaxGain, midChosenMidValue, midChosenAttributeIndex = findBestGain(allRecords, listCatIndex, attributeChoice) 
...
for i in range(0, len(trainRecordsList)): # Run for that many sets of training data
	...
	# Runs for all ten random forests trees
	resultMatrix = np.zeros((len(testRecordsList[i]),2) , dtype=int)
	...
	for j in range(0,11): # For each random forest's tree
		
		uniqueNoOfRec = int(0.63 * noOfTrainingRows)

		duplicateNoOfRec = int(0.37 * noOfTrainingRows)

		npUniqueTrainRec = np.random.choice(trainRecordsList[i],uniqueNoOfRec,replace=False)
		#npUniqueTrainRec = np.random.sample(trainRecordsList[i],uniqueNoOfRec)
		...
		# Runs for all the test records results
		for index in range(0,len(testRecordsList[i])): # Extracting an index from a list of training data indexes 
			
			npTestRecordsListRow = recordsFromFile[testRecordsList[i][index]]
			resultValue = testingModule(npTestRecordsListRow, rootNode, listCatIndex)
			resultMatrix[index][int(resultValue)] += 1  

	resultsList = []
	resultsList = np.asarray(resultsList)
	resultsList = np.argmax(resultMatrix,axis=1)
