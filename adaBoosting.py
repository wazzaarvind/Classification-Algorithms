...
def readFile():
	...
	return npAllRecords, listObjects, fileName
...
def crossValidData(npRecFromFile): # Accepts a numpy.ndarray()
	...
	allRecords, listCatIndex, fileName2 = readFile() # recordsFromFile is a numpy object
...
def runTreeBuild(treeDepth, parentNode, listOfCats, allRecords): # allRecords is of type numpy.ndarray()
	...
	if(impurityStatus == 0):
		...
		# Classify the leaf node; pass the records in pandas format
...
recordsFromFile, listCatIndex, fileName = readFile() # recordsFromFile is a numpy object
...
for i in range(0, len(trainRecordsList)): # Run for that many sets of training data
	...
	noOfTrainingRows = len(trainRecordsList[i])
	
	noOfTrees = 0

	weightOfTrainingRecords = [1/noOfTrainingRows for li in range(0,len(trainRecordsList[i]))]

	listOfTreeImportance = []
	listOfRootNodes = []
	...
	while(noOfTrees < 12):
		# Generate 11 trees and populate a list with root nodes

		uniqueNoOfRec = int(math.ceil(0.63 * noOfTrainingRows))

		duplicateNoOfRec = int(0.37 * noOfTrainingRows)

		npUniqueTrainRec = np.random.choice(trainRecordsList[i],uniqueNoOfRec,replace=False,p=weightOfTrainingRecords)
		
		npUniqueTrainRec = list(npUniqueTrainRec)

		npDuplicateTrainRec = np.random.choice(npUniqueTrainRec,duplicateNoOfRec,replace=True)
		
		npDuplicateTrainRec = list(npDuplicateTrainRec)

		npTempTrainRecords = npUniqueTrainRec + npDuplicateTrainRec

		npTempTrainRecords.sort()

		npTrainRecords = recordsFromFile[npTempTrainRecords]
		
		rootNode = runTreeBuild(-1, None, listCatIndex, npTrainRecords) # Build the decision tree based on a list of training data

		resultListTraining = []

		for index in range(0, len(npTempTrainRecords)):
			
			singleTrainRec = recordsFromFile[npTempTrainRecords[index]]
			resultValue = testingModule(singleTrainRec, rootNode, listCatIndex)
			resultListTraining.append(resultValue)


		groundTruthTraining = recordsFromFile[npTempTrainRecords,-1]

		wrongRowsMark = (abs(groundTruthTraining - resultListTraining))

		errorValue = sum(wrongRowsMark * weightOfTrainingRecords)

		onetreeImportance = (1/2)*math.log(((1-errorValue)/errorValue))
	
		if(errorValue < 0.5):

			listOfTreeImportance.append(onetreeImportance)

			listOfRootNodes.append(rootNode)

			noOfTrees += 1


		for q in range(0,len(wrongRowsMark)):
			wrongRowsMark[q] = math.exp(-1 * onetreeImportance * wrongRowsMark[q])

		weightOfTrainingRecords = weightOfTrainingRecords * wrongRowsMark

		sumOfNewWeights = 0

		sumOfNewWeights = sum(weightOfTrainingRecords)

		weightOfTrainingRecords = [t/sumOfNewWeights for t in weightOfTrainingRecords ]


	for oneRecPos in testRecordsList[i]: # Extracting an index from a list of training data indexes 
		
		npTestRecordsListRow = recordsFromFile[oneRecPos]

		resultListTesting = []

		for eachTree in listOfRootNodes:

			resultListTesting.append(testingModule(npTestRecordsListRow, eachTree, listCatIndex))

		resultListTesting = [-1 if z == 0 else z for z in resultListTesting]
		
		resultListTesting = np.array(resultListTesting)

		finalWeightedClass = resultListTesting * listOfTreeImportance

		sumOffinalWeightedClass = sum(finalWeightedClass)
		
		if(sumOffinalWeightedClass > 0):
			finalResultClass = 1
		
		else:
			finalResultClass = 0

		resultsList.append(finalResultClass)
